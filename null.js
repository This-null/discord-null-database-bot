...
const yarramm = require("./models/siktim.js")
...
setInterval(function(){
  let urll = new MessageEmbed().setColor("8c5df8").setAuthor("URL İnfo")
  client.guilds.cache.get(ayarlar.guildID).channels.cache.get(channel).send(urll.setDescription(`Vanity URL : \`\`${ayarlar.sunucuURL}\`\` olarak güncellendi.`))
  },900000);
  setInterval(function(){
      URL()
  },1350)
...
client.on("message", async message => {
  ...
  if(command === "ryükle") {
    if (!args[0] || isNaN(args[0])) return message.channel.send("`Veritabanından rol seç.`");

    Database.findOne({guildID: ayarlar.guildID, roleID: args[0]}, async (err, roleData) => {
      if (!roleData) return message.channel.send("`Veri Bulunamadı.`");
      message.react("☑️");
...
function otokayıt (guildID) {
  let guild = client.guilds.cache.get(ayarlar.guildID);
  let embed = new MessageEmbed().setColor("fbfd03").setAuthor("Auto backup")
  if (!guild) return;
...
client.on("roleDelete", role => {
  let embed = new MessageEmbed().setColor("fbfd03").setAuthor("Rol Silme").setFooter("null 💛").setTimestamp();
  client.channels.cache.get(ayarlar.rolLog).send(embed.setDescription(
    "**Silinen Rol ID**: `" + role.id + "`\n" + 
     "**Silinen Rol İsim**: `" + role.name + "`\n" +
      "**Rolü Kurmak için**: `.ryükle " + role.id + " `"
));
});

client.on("channelDelete", channel => {
  let embed = new MessageEmbed().setColor("fbfd03").setAuthor("Kanal Silme").setFooter("null 💛").setTimestamp();
  client.channels.cache.get(ayarlar.rolLog).send(embed.setDescription(
    "**Silinen Kanal ID**: `" + channel.id + "`\n" + 
    "**Silinen Kanal İsim**: `" + channel.name + "`\n" +
     "**Kanalı Kurmak için**: `.kyükle " + channel.id + " `" 
));
});
